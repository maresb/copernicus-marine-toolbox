...
class SUBSET_HELP:
    ...
    MINIMUM_DEPTH_HELP = (
        ...
        " Requires a float within this range: [0; 10000]."
    )
    MAXIMUM_DEPTH_HELP = (
    ...
    START_DATETIME_HELP = (
        ...
        "common format parsed by pendulum. See https://pendulum.eustace.io/docs/"
    )
    END_DATETIME_HELP = (
    ...
    COORDINATES_SELECTION_METHOD_HELP = (
        ...
        "inside’, the retrieved selection will be inside the requested interval and "
        "an error will raise if there doesn’t exist the values. If ‘nearest’, the "
        "returned interval extremes will be the closest to what has been asked for. "
        "A warning will be displayed if outside of bounds. If ‘outside’, the extremes"
        " will be taken to contain all the requested interval. A warning will also be"
        " displayed if the subset is outside of the dataset bounds."
    )
...
class GET_HELP:
    ...
    FILTER_WITH_GLOBBING_PATTERN_HELP = (
        "A pattern that must match the absolute"
    ...
    CREATE_FILE_LIST_HELP = (
        ...
        "action will be performed. Please find more information in the README."
    )
    SYNC_HELP = (
